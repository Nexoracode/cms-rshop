...
export function mapAPIToLocalForm(initialData: any, formType: string): PromotionForm {
  const allowedProducts =
    (initialData?.allowed_products || []).map((p: any) => {
      if (p?.product_id) return { product_id: p.product_id, variants: p.variants ?? null };
      if (typeof p === "object" && p.productId) return { product_id: p.productId, variants: p.variantIds ?? null };
      return { product_id: p, variants: null };
    }) || [];

  const firstOrderCond = !!initialData?.conditions?.some((c: any) => c.type === "first_order");
  ...
  return {
    ...
    starts_date: initialData?.startsAt || initialData?.start_date || "",
    ends_date: initialData?.endsAt || initialData?.end_date || "",
...
export function mapLocalFormToAPI(
  ...
  if (form.allowed_products?.length) {
    conditions.push({ type: "product", product_ids: form.allowed_products.map(p => p.product_id) });
    conditions.push({ type: "variant", variant_ids: form.allowed_products.flatMap(p => p.variants ?? []) });
  }
  ...
  return {
    ...
    starts_at: form.starts_date,
    ends_at: form.ends_date,
