...
const PromoBannerFormModal: React.FC<Props> = ({
  defaultValues,
  isOpen,
  onOpenChange,
}) => {
  ...
  const { form, errors, setForm, handleFieldChange, reset, submit } = useForm(
    initialForm,
    {
      onValidate: (data: any) => promoBnannerValidation(data, showLink),
    ...
    <BaseModal
      ...
      triggerProps={
        defaultValues?.id
          ? null
          : {
              title: "+ افزودن بنر اطلاع رسانی تبلیغاتی",
              className: "bg-secondary-light text-secondary",
            }
      }
      title={defaultValues?.id ? "ویرایش بنر" : "افزودن بنر"}
