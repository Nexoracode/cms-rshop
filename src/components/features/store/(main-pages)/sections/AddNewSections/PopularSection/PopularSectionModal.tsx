...
import { popularValidation } from "./popular-section-validation";
...
const PopularSectionModal: React.FC<Props> = ({
  defaultValues,
  isOpen,
  onOpenChange,
}) => {
  ...
  const { form, errors, setForm, handleFieldChange, reset, submit } = useForm(
    initialForm,
    {
      onValidate: (data: any) => popularValidation(data, showLink),
  ...
  return (
    <BaseModal
      ...
      title={defaultValues?.id ? "ویرایش محبوب ترین محصولات" : "افزودن محبوب ترین محصولات"}
...
export default PopularSectionModal;
