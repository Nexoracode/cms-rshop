...
const TaskerVehicles = ({ taskerID = 0, onClose }: TaskerDocsProps) => {
    ...
    const getTaskerDocsHandler = async () => {
        ...
        setDataList([])
        const res = await sendRequestWithLoading(`/admin/tasker/${taskerID}/vehicles?page=${activePage}`, {}, "post", "Getting Vehicles", false, false)

        if (res?.data?.data) {
            const { taskerVehicles, size, page, total, totalPages } = res.data.data[0]
            setPaginations({ size, page, total, totalPages })
            setDataList(taskerVehicles)
        }
    }
    ...
    const updateDescriptionHandler = async (data: string) => {
        ...
        const res = await sendRequestWithLoading(`/admin/tasker/${taskerID}/vehicles/${id}`, { description: data }, "put", "Update Description", false, false)
        if (res?.data?.data) {
            closeOnBoxHandler()
    ...
    const deleteTaskerVerifyHandler = (id: number) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const res = await sendRequestWithLoading(`/admin/tasker/${taskerID}/vehicles/${id}`, {}, "delete", "Deleting Tasker Vehicle", false, false)
        ...
        <WrapperContents
            ...
            contentTable={
                <Table
                    ...
                    cols={["ID / Email Tasker", "Service Title", "Vehicle Title", "CreatedAt", "Description"]}
                    hasDatas={dataList?.length ? true : false}
                    paginationProps={paginations}
                    navbar={{
                        headerTitle: "Tasker Vehicles",
                ...
                >
                    {
                        dataList && dataList.length
                            ?
                            dataList.map(items => {
                                const { id, description, tasker, created_at, service_vehicle } = items
                                ...
                                return (
                                    <tr key={id} className={`bg-[var(--background)] hover:opacity-75 transition-all duration-300 border-t border-t-gray-300 ${(editType === "DESCRIPTION" && taskerDocsDescription.id === id) ? "bg-[var(--light-primary)] animate-pulse hover:animate-none" : ""}`}>
                                        ...
                                        <RowTable title={service_vehicle.service_title} />

                                        <RowTable title={service_vehicle.vehicle_title} />
